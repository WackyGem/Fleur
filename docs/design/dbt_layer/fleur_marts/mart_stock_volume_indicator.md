# mart_stock_volume_indicator 设计
...
- 目标 SQL：`pipeline/elt/models/marts/mart_stock_volume_indicator.sql`
- 目标 YAML：`pipeline/elt/models/marts/mart_stock_volume_indicator.yml`

## 1. 模型定位

A 股股票日频成交量形指标 mart。模型只暴露均量字段，作为成交量形指标的稳定消费接口。

均量当前由 Furnace MA calculation 表和 `int_stock_ma_daily` wrapper 提供。本 mart 按语义把这些字段从价格趋势指标中拆出，避免把成交量指标混入 `mart_stock_trend_indicator`。
...
uv run dbt build --project-dir elt --profiles-dir elt --select mart_stock_volume_indicator
