...
- 已有 `int_stock_financial_valuation` 可按 `report_date <= trade_date` as-of 取最近报告期，提供 PE/PB、每股净资产、ROE 和 ROAE。
...
- `roe` 和 `roae` 使用比率口径，不乘以 `100`；这两个字段来自 `int_stock_financial_valuation`。
...
- `pe_static`、`pe_ttm`、`pe_forecast`、`pb_mrq`、`book_value_per_share`、`roe`、`roae` 均来自 `int_stock_financial_valuation`。
...
- `roe` 和 `roae` 沿用 `int_stock_financial_valuation` 的比率口径，不乘以 `100`；归母净利润为负时允许输出负值。
...
6. 财报期指标与股息率：`pe_static`, `pe_ttm`, `pe_forecast`, `pb_mrq`, `book_value_per_share`, `roe`, `roae`, `dy_static`, `dy_ttm`
...
2. `int_stock_financial_valuation`：按最近 `report_date <= trade_date` 提供 `pe_static`, `pe_ttm`, `pe_forecast`, `pb_mrq`, `book_value_per_share`, `roe`, `roae`。
...
- 不在 mart 内从 `stg_eastmoney__income_*` 或 `stg_eastmoney__balance` 直接计算 PE/PB、每股净资产、ROE 或 ROAE。
...
- 定向数据测试：
  ...
  - `roe` 和 `roae` 来源值应与最近报告期 `int_stock_financial_valuation` 对应字段一致。
