...
A 股股票日频行情 mart 宽表。模型以 `int_stock_quotes_daily_unadj` 为主事实表，保留其全部原有字段，并补充面向分析消费的前复权/后复权价格、日频交易指标、市值股本指标、估值指标和股息率指标。
...
- Join 策略：以未复权行情表为左表。缺少复权行情或财报期估值输入时，保留未复权行情行，对应扩展字段输出 `NULL`。
...
4. 前成交量、成交与交易指标：`prev_volume`, `volume`, `amount`, `turnover_rate`, `turnover_rate_actual`, `pct_amplitude`, `pct_change`
...
7. 财报期指标与股息率：`pe_static`, `pe_ttm`, `pe_forecast`, `pb_mrq`, `book_value_per_share`, `roe`, `roa`, `roaa`, `roae`, `dy_static`, `dy_ttm`
...
- 定向数据测试：
  ...
  - `prev_volume`, `turnover_rate`, `turnover_rate_actual`, `pct_amplitude`, `pct_change`, `limit_up_price`, `limit_down_price`, `a_market_cap`, `a_float_market_cap`, `a_free_float_market_cap`, `a_shares`, `a_float_shares`, `a_free_float_shares`, `dy_static`, `dy_ttm` 应与 `int_stock_quotes_daily_unadj` 对应字段一致。
