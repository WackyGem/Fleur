# mart_stock_momentum_indicator 设计
...
- 目标 SQL：`pipeline/elt/models/marts/mart_stock_momentum_indicator.sql`
- 目标 YAML：`pipeline/elt/models/marts/mart_stock_momentum_indicator.yml`
...
| KDJ 参数 | `int_stock_kdj_daily` | `kdj_rsv_window`, `kdj_k_smoothing`, `kdj_d_smoothing` |
...
- 第一版固定 canonical `KDJ(9,3,3)`。
- 参数字段应为 `kdj_rsv_window = 9`、`kdj_k_smoothing = 3`、`kdj_d_smoothing = 3`。
...
uv run dbt build --project-dir elt --profiles-dir elt --select mart_stock_momentum_indicator
