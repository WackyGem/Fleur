...
| `end_date` | `END_DATE` | `Date` | 前十大流通股东名单对应的报告期截止日期；保留 source-local 字段名以匹配股东披露期语义。 |
| `holder_rank` | `HOLDER_RANK` | `Int64` | 东方财富返回的流通股东名次；不限制为 1 至 10。 |
| `holder_eastmoney_code` | `HOLDER_NEW` | `String` | 东方财富本地股东标识文本，可能为数字编码或股东姓名，不作为跨源稳定股东 ID。 |
...
- `END_DATE` 保持 `end_date`，不改为财务报表口径的 `report_date`。
...
- 模型级组合唯一：`security_code`, `end_date`, `holder_rank`, `holder_eastmoney_code`, `holder_name`, `shares_type`。
- `security_code`: `not_null`，`cn_security_code_format`。
- `end_date`: `not_null`。
- `holder_rank`: `not_null`。
- `holder_eastmoney_code`: `not_null`。
