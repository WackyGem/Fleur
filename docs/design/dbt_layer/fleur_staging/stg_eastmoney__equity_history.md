...
| `report_date` | `END_DATE` | `Date` | 股本变动截止日，使用 glossary canonical 字段。 |
...
- 现有第一版 staging 已输出 `security_code`, `security_local_code`, `exchange_code`, `report_date`；扩展宽字段时需同步扩展 glossary 或 local metadata。
