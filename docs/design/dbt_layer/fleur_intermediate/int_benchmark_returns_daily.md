...
本模型只输出日频事实所需字段，不输出本地指数代码和交易所代码；需要这些维度时通过 `security_code` join `int_benchmark_basic_snapshot`。benchmark 标识沿用 canonical `security_code`，不引入平行 benchmark key。

## 2. 数据粒度与依赖

- 直接依赖：`int_benchmark_basic_snapshot`、`int_index_quotes_daily`。
- 粒度：一行代表一个 benchmark 指数 `security_code` 在一个 `trade_date` 的价格指数日收益。
...
| 字段 | 来源/派生 | 类型建议 | 设计说明 |
|---|---|---|---|
| `security_code` | `int_benchmark_basic_snapshot` | `String` | canonical 指数代码，benchmark 标识。 |
...
- 不引入 `benchmark_key` 或 `benchmark_name`，benchmark 即指数。
