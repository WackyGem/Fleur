...
同 [action/field](jiuyan__action_field.md)，需 `token` + `cookie` + `platform` + `timestamp` 头部。`timestamp` 必须在每次请求发送时动态计算为当前 Unix 毫秒时间戳，例如 Python 中使用 `str(int(time.time() * 1000))`；不要写死复测用的固定值。
...
- 使用本地 `.env` 中 `JIUYAN_TOKEN` / `JIUYAN_COOKIE`，并按有效请求要求携带测试用固定 `timestamp=1779217286540`，对 `start=0/1/2/3, limit=500` 发起请求；生产实现必须动态计算当前 Unix 毫秒时间戳。
