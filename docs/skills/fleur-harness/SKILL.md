---
name: fleur-harness
description: mono-fleur 的项目 harness 工程技能。用于把复杂开发、重构、质量治理、文档维护和 agent 可读性工作落到仓库内的 docs、系统地图、测试、脚本和质量门禁中；尤其适用于需要整理 AGENTS.md、docs/systems、ADR/RFC/plan/optimize/job report、架构边界或长期维护规则的任务。
---
...
- `docs/README.md`：docs 总入口，提供按系统和按文档类型的双入口阅读顺序。
- `docs/systems/`：按系统和产品线组织当前事实、代码根、职责边界、运行入口、质量门禁和相关文档指针；新增长期系统或应用时优先维护这里。
...
2. `docs/README.md`：确认 docs 当前双入口、目录职责、状态模型和归档规则。
...
10. 历史材料：`docs/RFC/archive/` 和 `docs/plans/archive/` 只用于追溯背景；不得把归档内容当作当前事实，除非当前系统地图、当前代码或活跃文档再次确认。
