---
name: fleur-harness
description: mono-fleur 的项目 harness 工程技能。用于把复杂开发、重构、质量治理、需求投递、文档维护和 agent 可读性工作落到仓库内的 docs、系统地图、测试、脚本和质量门禁中；尤其适用于需要整理 AGENTS.md、docs/systems、ADR/RFC/plan/archive/optimize/job report、架构边界或长期维护规则的任务。
---
...
核心原则：`AGENTS.md` 是仓库入口地图，`docs/systems/` 是按系统组织的当前事实地图，`docs/` 是记录系统，测试和脚本是可执行约束。不要把长期规则只留在一次性提示或聊天上下文里。
...
1. 先读地图：从 `AGENTS.md`、`docs/README.md` 和相关 `docs/systems/` 入口确认项目结构、命令、边界和质量门禁。
...
- `docs/README.md`：docs 总入口，提供按系统查事实和按文档类型追溯的入口。
- `docs/systems/`：按系统和产品线组织当前事实、代码根、职责边界、运行入口、质量门禁和相关文档指针；新增长期系统或应用时同步维护。
...
- `docs/debt/`：技术债和实现漂移记录入口。
- `docs/optimize/`：质量扫描、维护性审计和治理建议入口。
...
2. `docs/README.md`：确认 docs 当前系统事实入口、文档类型入口、目录职责、状态模型和归档规则。
3. `docs/systems/README.md`，再读相关 `docs/systems/*.md`：确认当前系统代码根、职责边界、运行入口、质量门禁和相关文档指针。
4. `docs/architecture/` 中与该系统相关的当前架构文档：确认跨模块边界、依赖方向和禁止模式。
...
10. 历史材料：`docs/RFC/archive/`、`docs/plans/archive/`、`docs/debt/` 和 `docs/optimize/` 只用于追溯背景；不得把归档内容当作当前事实，除非系统地图、当前代码或活跃文档再次确认。
...
- 新增系统、应用、服务或长期子域时，同时新增或更新系统地图。
- 更新长期规则时，检查是否也需要更新 `AGENTS.md`、系统地图、架构文档、相关 skill 或测试。
- 同一规则只保留一个权威位置，其他位置使用简短指针。
- 如果代码已经否定旧文档，优先修文档；如果文档表达的是仍应遵守的约束，优先补测试。
- 历史文档不要伪装成当前状态：归档、标注日期或写明“当前实现已变化”。
- 不变更已归档文件的内容。`archive/` 下的 RFC、plan、optimize、report 等历史文件只用于追溯；若当前事实变化，应更新当前文档、系统地图、ADR、active plan 或新增后续记录，而不是改写归档内容。
