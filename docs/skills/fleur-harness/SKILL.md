...
- `docs/plans/`：复杂执行计划、阶段拆分、验收标准；活跃计划保留在顶层，完成后归档。
...
2. `docs/architecture/scheduler-architecture.md`：确认当前 scheduler 总体架构和主要数据流。
3. `docs/architecture/scheduler-module-boundaries.md`：确认模块职责、依赖方向和禁止模式。
4. `docs/ADR/README.md`，再读相关 `docs/ADR/*.md`：确认已经接受且仍应遵守的长期决策。
5. 活跃设计与执行文档：先读相关 `docs/RFC/*.md`，再读相关 `docs/plans/*.md` 和 `docs/optimize/*.md`，确认目标、非目标、阶段和验收标准。
6. 运行事实：涉及回填、重跑、数据核验或生产结果时，读 `docs/jobs/dagster-backfill-2026.md` 和相关 `docs/jobs/reports/*.md`。
7. 接口与数据事实：涉及远端 API、schema、样例或数据契约时，读相关 `docs/references/openapi/`、`docs/references/remote_endpoint/`、`docs/references/data_dict/`、`docs/references/remote_server/`。
8. 复用手册：涉及具体 agent 操作时，读相关 `docs/skills/*/SKILL.md`，并遵守其中更具体的 runbook。
9. 历史材料：`docs/RFC/archive/` 和 `docs/plans/archive/` 只用于追溯背景；不得把归档内容当作当前事实，除非当前代码或活跃文档再次确认。
