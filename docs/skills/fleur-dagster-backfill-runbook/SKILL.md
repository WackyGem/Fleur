...
1. Source 到 ClickHouse raw 的回填优先使用统一入口 `backfill__fetch_sources_to_raw_job`。
2. 只有调试单个资产、验证单个分区或统一入口暂不覆盖的临时操作，才使用精确 asset selection。
3. 判断目标是否分区。
4. 从 [references/backfill-matrix.md](references/backfill-matrix.md) 里选命令模板。
5. 需要时先用 `uv run dg list defs --target-path scheduler --json` 验证选择。
6. 先跑 `dry_run: true`，确认计划后再执行真实回填。
