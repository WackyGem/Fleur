...
- 数值输出默认使用普通十进制和千分位，不用科学计数法；如果原始工具输出科学计数法，报告中要转成人可读数字，并说明这只是展示格式，不代表类型溢出。
...
- 不要把超长机械聚合 SQL 原样塞进报告，例如全字段 NULL/min/max 聚合。此类结果应展开到“字段画像”和“关键 SQL 证据摘要”，附录只保留抽样、行数、候选键、格式分布、枚举 top values 等 reviewer 真正需要复核的查询。
