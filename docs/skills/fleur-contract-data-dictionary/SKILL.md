...
description: mono-fleur 的 raw 层数据契约、字段 glossary、dbt sources.yml 和 data_dict 生成工作流。用于新增或修改 pipeline/contracts/datasets、维护 external_description_zh/description_zh、运行 fleur-contracts 校验与生成、修复生成的数据字典漂移。
---

# Fleur Contract Data Dictionary

当任务涉及 `pipeline/contracts`、`pipeline/contract_tools`、`docs/references/data_dict`、dbt `sources.yml` 的 raw 字段事实、中文字段描述或生成物同步时，使用这个 skill。
...
- 当前 raw scope 计划：`docs/plans/0021-contract-scope-raw-layer-cleanup-plan.md`
- 历史实施计划：`docs/plans/0018-data-contract-registry-and-staging-layer-plan.md`
...
- raw 字段变更后重新生成 dbt `sources.yml` 和 data_dict。
