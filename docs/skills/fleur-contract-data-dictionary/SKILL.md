...
description: mono-fleur 的数据契约、字段 glossary、dbt YAML 和 data_dict 生成工作流。用于新增或修改 pipeline/contracts/datasets、维护 external_description_zh/description_zh、运行 fleur-contracts 校验与生成、修复生成的数据字典漂移。
---

# Fleur Contract Data Dictionary

当任务涉及 `pipeline/contracts`、`pipeline/contract_tools`、`docs/references/data_dict`、dbt `sources.yml` / `staging.yml` 的字段事实、中文字段描述或生成物同步时，使用这个 skill。

## 核心边界

- 字段事实源是 `pipeline/contracts/datasets/*.yml` 和 `pipeline/contracts/glossary/*.yml`。
- `docs/references/data_dict/*.md`、`pipeline/elt/models/sources.yml`、`pipeline/elt/models/staging/staging.yml` 是生成物，不作为字段事实源。
- 修改字段事实时先改 contract/glossary，再运行生成器；不要手工修 generated data_dict。
...
- dbt YAML 生成器：`pipeline/contract_tools/src/fleur_contracts/adapters/dbt.py`
- 当前设计背景：`docs/RFC/0010-data-contract-registry-and-contract-tools.md`
- 实施计划：`docs/plans/0018-data-contract-registry-and-staging-layer-plan.md`
- 中文描述修复计划：`docs/plans/0019-contract-zh-description-quality-remediation-plan.md`

## 工作流

1. 先读相关 dataset contract 和 glossary，不从 generated data_dict 反推字段事实。
2. 确认字段链路：`source.fields` -> `parquet.fields` -> `clickhouse_raw.fields` -> 可选 `dbt_staging.fields`。
3. 修改字段、类型、nullable、`from` 映射、`glossary_key` 或中文描述。
4. 运行 contract 静态校验。
5. 写回生成物。
6. 再运行生成物一致性检查和 contract tests。
...
- active `dbt_staging.fields[].from` 必须指向 raw field。
- active stg 字段优先设置 `glossary_key`；没有 glossary 的技术字段必须显式 `canonical_exempt: true` 和 `exempt_reason`。
- `LowCardinality(String)` 字段必须写 `reason`。
- raw/stg 字段变更后重新生成 dbt YAML 和 data_dict。
...
如果改动影响 scheduler raw sync adapter 或 dbt models，再按 `AGENTS.md` 跑对应 scheduler/dbt 最小门禁。
