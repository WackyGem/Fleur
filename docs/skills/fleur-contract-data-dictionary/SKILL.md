...
- dbt `staging.yml`、`stg_*.sql`、stg 字段描述、tests 和 meta 由 `pipeline/elt` 项目维护，不写入数据契约。
...
3. 修改字段、类型、nullable、`from` 映射或中文描述。
...
- 支持的 contract Parquet 类型包括：`string`、`date32[day]`、`date32`、`bool`、`int8`、`int32`、`int64`、`float64`、`double`、`timestamp[s]`、`timestamp[s, tz=UTC]`、`timestamp[ms]`、`timestamp[ms, tz=UTC]`、`timestamp[ns]`、`timestamp[ns, tz=UTC]`、`time32[ms]`。
...
dbt canonical 字段描述、字段值格式、generic tests 和 staging lineage 由
`pipeline/elt/metadata/field_glossary.yml`、`pipeline/elt/models/staging/**`
和 `pipeline/elt/scripts/validate_field_glossary.py` 管理。
