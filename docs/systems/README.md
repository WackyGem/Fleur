...
本目录按系统和产品线组织 mono-fleur 的当前事实地图。它不替代 `ADR/`、`RFC/`、`plans/`、`jobs/`、`design/` 或 `references/`；这些目录仍按文档生命周期和事实类型保存权威内容。

使用方式：

1. 先从系统地图确认代码根、职责边界、运行入口和质量门禁。
2. 再跳转到相关 ADR、RFC、plan、job report、design 或 reference 文档。
3. 最后回到当前代码和测试验证事实，避免把历史设计当作当前实现。
...
| Racingline | `app/racingline/`（规划中） | [racingline.md](racingline.md) | Rearview 前端选股工作台 |
