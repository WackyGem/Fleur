...
2. 将 Step 1/2 的 `RuleVersionSpec`、Step 3 非 stale applied preview snapshot、Step 4 canonical `BacktestExecutionConfig`、Step 5 `period_key`、动态解析后的回测区间和 benchmark 固化为一次可复现 backtest run。
...
9. 首次从 Step 4 进入 Step 5 时默认使用动态 `近一年` 回测周期；用户可在 Step 5 重新选择 `近一年/近两年/近三年` 和 benchmark，并通过 `开始回测/重新回测` 创建新的异步 backtest run。
10. 完成默认动态周期 live smoke 和验收报告，证明 Step 1 到 Step 5 的真实执行流程连贯。
...
19. `app/racingline_new/src/routes/strategy-page.tsx` 当前已有 `1y/2y/3y` 周期选项，首次渲染默认 `1y`，benchmark 默认 `000300.SH`。
20. `app/racingline_new/src/features/strategy/execution.ts` 当前的 `buildBacktestDateRange()` 使用浏览器 UTC 日期按自然年回退生成 `start_date/end_date`；这不是 Step 5 的权威动态区间，因为它不感知交易日历、最新可用行情日或 benchmark 覆盖情况。
...
| G2 | Step 5 没有 first-class control plane。 | 新增 `strategy_backtest_run`，固化 rule/config/period/resolved range/benchmark/hash/status/current attempt。 | Phase 1 |
...
| G23 | Step 5 动态回测周期目前可能由前端本地日期计算。 | `1y/2y/3y` 必须作为 `period_key` 提交；Rearview 用交易日历、最新可用行情日和所选 benchmark 动态解析 `[start_date, end_date]`，并把解析结果固化到 run。前端不得把浏览器日期计算结果作为权威区间。 | Phase 0, Phase 1, Phase 2, Phase 5 |
| G24 | 修改周期或 benchmark 后的重新回测语义不完整。 | period/benchmark 改动只改变 Step 5 pending config，不自动执行；点击 `开始回测/重新回测` 必须生成新 `client_request_id` 和新 run，旧 run 作为历史快照保留。 | Phase 2, Phase 5 |

## 第一性原理闭环审视

Step 5 做完后要称为“策略回测闭环”，必须同时满足下面六个条件：

1. 输入闭环：Step 1/2 的 `RuleVersionSpec`、Step 3 non-stale snapshot、Step 4 canonical `execution_config`、Step 5 `period_key`、动态解析后的 frozen range、benchmark 和 UI display labels 都被一次性冻结，后续草稿变化不会污染已创建 run。
...
Review 结论：原计划已覆盖主链路，但在补齐 G16-G24 之前只能保证 happy path，不能保证完整闭环。补齐这些缺口后，Step 5 第一版可以形成“配置 -> 异步回测 -> 结果持久化 -> UI 展示”的闭环；“将成功 backtest 发布成正式运行策略”仍按非目标另起方案。
...
|---|---|---|---|
| 回测配置 | 周期、动态解析后的回测区间、业绩比较基准、开始/重新回测按钮状态。 | `strategy_backtest_run.period_key/start_date/end_date/benchmark_security_code`，以及 options/range resolver 返回的 pending config。 | 首次进入默认 `period_key = 1y`；`1y/2y/3y` 由 Rearview 动态解析区间；GET run 返回 frozen range、benchmark code、benchmark label；server validate benchmark 是否在允许列表内。 |
...
Step 5 live smoke 必须使用下面这组前端可录入规则作为端到端验收样例。该样例覆盖 Step 1 多 mart 过滤、Step 2 字段间比较/字段乘常数/分段加分、Step 4 默认仓位和风控配置、Step 5 异步回测与结果展示。

字段名以已落地 `engines/crates/rearview-core/config/metric_policy.yml` 和 `engines/crates/rearview-core/src/domain/rule.rs::representative_rule()` 为准。用户口径到 canonical 字段的映射固定为：

| 用户口径 | 验收使用字段 | 说明 |
|---|---|---|
| `KDJ J` / `J` | `kdj_j_value` | KDJ J 值。 |
| `amplitude` | `pct_amplitude` | policy logical metric；来源列为 `amplitude_pct`，百分数口径。 |
| `pct_change` | `pct_change` | policy logical metric；来源列为 `change_pct`，百分数口径。 |
| `forward_close_price` | `close_price_forward_adj` | 前复权收盘价，验收中所有“前复权收盘价/forward close”均用该字段。 |
| `ema2_10` | `price_ema2_10` | 二重 EMA10。 |
| `avg_ma_14_28_57_114` | `price_avg_ma_14_28_57_114` | 组合 MA 14/28/57/114。 |
| `avg_ma_3_6_12_24` | `price_avg_ma_3_6_12_24` | 组合 MA 3/6/12/24。 |
| `boll_dn_20_2` | `boll_lower_20_2` | 已落地 BOLL 下轨字段使用 `lower` 命名，不使用 `dn`。 |
| 其他字段 | 原字段名 | `volume`、`prev_volume`、`volume_ma_5`、`rsi_6`、`close_down_streak_days`、`n_structure_20_second_low_ratio`、`price_ma_20`、`price_ma_60`、`price_ma_114`、`price_ma_250` 均直接使用现有字段。 |

Step 1 过滤条件录入为一个 AND 组：
...
Step 2 得分条件录入为 conditional points，分数 clamp 沿用后端 canonical `[0, 100]`：
...
KDJ 两条得分规则必须按互斥分段实现：`kdj_j_value < -15` 命中 `+25`；`-15 <= kdj_j_value < -10` 命中 `+15`。不得把 `J < -15` 同时计入 `J < -10` 的 `+15`。

Step 4 验收配置：

1. 仓位和费率采用 `app/racingline_new/src/routes/strategy-page.tsx` 当前默认值：`initialCapital = 1_000_000`、`buyTopN = 5`、`maxPositions = 5`、`singlePositionLimitPercent = 10`，佣金、印花税、过户费、滑点使用默认模板值。
2. 风控条件全部选择/启用，参数值使用 Step 4 UI 默认值和 validate API 返回的 canonical config。
3. 指标止损如需选择主图指标，优先使用 Step 4 已支持的趋势均线主图指标集合：MA、MA 组合、EMA。

Step 5 验收动作：

1. 首次进入 Step 5 保持默认 `近一年 + 000300.SH`，创建 backtest run。
2. 成功后切换到 `近两年` 或 `近三年`，并切换另一个 benchmark，确认旧结果标记为历史快照。
3. 点击 `重新回测` 创建新 run，确认新 run 的 `period_key`、benchmark 和 frozen range 与 pending config 一致。

截图证据链必须写入 `docs/references/screenshots/racingline/<date>/`，并由验收报告链接：

1. `step5-acceptance-01-step1-filters.png`：Step 1 展示上述 10 条过滤条件，字段名为 canonical 字段。
2. `step5-acceptance-02-step2-scoring.png`：Step 2 展示上述 7 条得分条件和分数。
3. `step5-acceptance-03-step3-preview.png`：Step 3 preview 成功且 snapshot 非 stale。
4. `step5-acceptance-04-step4-defaults-risk.png`：Step 4 展示默认仓位、默认费率和全部风控条件。
5. `step5-acceptance-05-step5-running.png`：Step 5 创建 run 后展示 queued/running/progress 状态。
6. `step5-acceptance-06-step5-succeeded-nav.png`：Step 5 succeeded 后展示真实策略净值和 benchmark 净值。
7. `step5-acceptance-07-step5-rebalance-performance.png`：Step 5 展示真实调仓记录和策略业绩侧栏。
8. `step5-acceptance-08-step5-rerun.png`：修改周期或 benchmark 后旧结果为历史快照，重新回测生成新 run。
...
Step 3 non-stale PreviewSnapshot
  ...
  + Step 5 period_key/benchmark
  -> GET /rearview/strategy-backtests/options displays pending dynamic range
  -> POST /rearview/strategy-backtests
  -> resolve dynamic range from trading calendar, latest available quote date, and benchmark coverage
  -> validate rule/config/period/benchmark/resolved range and compute hashes
...
3. Worker 只从 PostgreSQL immutable snapshot 读取 rule/config/period/resolved range/benchmark；不得从前端当前草稿、Step 3 preview rows 或市场默认模板重新推导。
...
17. `period_key` 只允许 `1y`、`2y`、`3y`；首次从 Step 4 进入 Step 5 且当前草稿无 run 时，UI pending config 默认 `period_key = 1y`。
18. 动态区间由 Rearview 解析：`end_date` 取不晚于请求时点的最新完整交易日，并同时满足基础行情和所选 benchmark return 可用；`start_date` 由 `end_date` 回退 N 个自然年后的第一个可交易日解析得到。解析结果写入 run 后不可变。
19. 前端可以展示 options/range resolver 返回的 pending `resolved_start_date/resolved_end_date`，但 create 时 Rearview 必须重新解析并固化最终区间；如果期间最新交易日变化，以 create 时的解析结果为准，GET run 返回的 frozen range 是展示和计算权威。
20. 用户修改 `period_key` 或 benchmark 只改变 pending config，不自动触发回测；点击 `开始回测/重新回测` 才创建新 run。
21. `重新回测` 不复用旧 run、不覆盖旧 result attempt；它创建新 run，新 run 的 request hash 必须包含 `period_key`、resolved range 和 benchmark。
...
9. 固定 Step 5 动态周期解析 contract：
   - `period_key` 只允许 `1y | 2y | 3y`。
   - `end_date` 取最新完整交易日，并满足基础行情和所选 benchmark return 可用。
   - `start_date` 取 `end_date` 回退 N 个自然年后的第一个可交易日。
   - create 阶段必须用后端 resolver 重新解析 range；前端传入的日期只能作为显示 hint 或一致性诊断，不能作为权威输入。
10. 增加 range resolver fixture，覆盖：
   - 请求日是交易日但数据未闭合时，`end_date` 回退到上一完整交易日。
   - 请求日是非交易日时，`end_date` 回退到上一完整交易日。
   - benchmark 缺少最新日 return 时，`end_date` 回退到 benchmark 与基础行情共同可用的最后交易日。
   - `1y/2y/3y` 的 `start_date` 都落在可交易日，且 `start_date <= end_date`。
11. 固定 live smoke 样本规则、动态周期、benchmark、TopN、maxPositions 和 Step 4 风控参数。
...
7. 后端 range resolver 测试固定 `1y/2y/3y` 动态区间和 benchmark 覆盖回退规则。
...
5. `data_preflight_snapshot` 至少保存：
   ...
   - resolved range。
   - range resolution snapshot id/hash。
...
8. create/get roundtrip 能保留 `period_key`、`range_as_of_date`、`range_resolved_at`、`range_resolution_snapshot` 和 frozen `start_date/end_date`。
...
1. 新增 API：
   - `GET /rearview/strategy-backtests/options`
   - `POST /rearview/strategy-backtests`
   - `GET /rearview/strategy-backtests/{strategy_backtest_run_id}`
2. `GET /options` handler 执行：
   - 返回 Step 5 允许的 benchmark 列表，包含 `security_code`、`label`、`is_default` 和 availability status。
   - 返回 `1y/2y/3y` period options，包含 `period_key`、`label`、`resolved_start_date`、`resolved_end_date`、`latest_available_trade_date`、`benchmark_security_code` 和 `range_resolution_snapshot`。
   - 默认 benchmark 沿用现有 UI 默认 `000300.SH`，除非该 benchmark 在当前数据覆盖下不可用；不可用时返回 validation status，不由前端静默换成其他 benchmark。
   - 接受可选 `benchmark_security_code` 和 `as_of_date` 查询参数；普通 UI 不传 `as_of_date`，测试可用固定 `as_of_date` 做 deterministic fixture。
   - 不执行完整 screening、simulation 或绩效计算。
3. `POST` handler 执行：
   - validate `period_key in ('1y', '2y', '3y')`。
   - 用 selected benchmark 和当前 server time 重新解析 dynamic range。
   ...
   - 计算 `request_hash`，输入必须包含 `period_key`、resolved `start_date/end_date` 和 benchmark。
...
5. 保持兼容：旧消息含 `portfolio_run_id` 时按 `portfolio_run` 处理。
6. `rearview-server` outbox dispatcher 支持 strategy backtest outbox；可以先共享同一 stream 和 subject。
7. `GET` 返回状态、hash、`period_key`、frozen range、benchmark、`config_summary`、`ui_display_snapshot`、summary、error、progress 和 `current_result_attempt_id`。
8. `config_summary` 必须覆盖 Step 4 页面配置：
   - initial cash。
   - buy signal TopN。
   - max positions。
   ...
   - take profit。
   - time stop loss。
9. GET run 必须返回 closure diagnostics：
   - `worker_attempt_no`。
   - `data_preflight_snapshot`。
   - `signal_summary`。
   - `data_coverage_summary`。
   - `compiled_sql_hash`。
   - `current_result_attempt_id`。
10. `重新回测` 必须生成新的 `client_request_id`；否则会被幂等逻辑识别为同一次 create。
11. Create request 第一版字段口径：
    - `rule`
    - `period_key`
    - `benchmark_security_code`
    - `top_n`
    ...
    - `client_request_id`
    - `ui_display_snapshot`
    - optional `range_hint`，只用于诊断 options 与 create 之间的解析漂移，不能覆盖后端 resolved range。
...
5. API 测试覆盖 validate 失败、hash mismatch、benchmark 不支持、空交易日历、period 不支持、idempotency、idempotency conflict 和 get not found。
6. GET run response 能直接驱动 Step 5 配置摘要、状态区和诊断区。
7. Options API 能返回 `1y/2y/3y` 的动态 resolved range，并随 benchmark 改变重新解析 range。
8. Create API 即使收到前端 `range_hint`，也以服务端重新解析的 `start_date/end_date` 为准。
...
3. TanStack Query hooks：
   - options query，按 selected benchmark 拉取 `1y/2y/3y` 动态 resolved range。
   - create mutation。
   - status polling query。
   - succeeded 后启用 result queries。
4. `BacktestPanel` 以现有原型为准，只替换数据来源，不重做页面结构。
5. `BacktestPanel` 状态：
   - `idle`：必须持有 `BacktestExecutionDraft`；展示 Step 4 draft 摘要、建仓规则、仓位、买入规则、卖出规则、费率、调仓、pending period、resolved range、benchmark。
...
9. `开始回测` 按钮只在以下条件满足时启用：
   ...
   - options query 已成功返回当前 benchmark 的 period options。
   - 当前 `period_key` 在 `1y/2y/3y` 内，且 options 返回的 resolved range 合法。
   - benchmark 合法。
10. 首次从 Step 4 进入 Step 5 的初始化规则：
   - 如果当前 Step 4 draft 没有对应 active backtest run，pending `period_key` 默认 `1y`。
   - pending benchmark 默认 `000300.SH`，与现有原型保持一致；如果 options API 标记该 benchmark 不可用，显示不可用状态并禁用开始回测，不静默替换 benchmark。
   - 初始页面先拉取 options，展示 `1y` 对应的动态 `resolved_start_date/resolved_end_date`。
11. 周期和 benchmark 交互规则：
   - 周期下拉只展示 options API 返回的 `1y/2y/3y`。
   - 切换周期只更新 pending `period_key` 和展示区间，不自动 create run。
   - 切换 benchmark 重新拉取 options 并更新 pending resolved range，不自动 create run。
   - 如果当前已有 succeeded run，且 pending `period_key` 或 benchmark 与 run snapshot 不一致，当前结果标记为历史快照，按钮显示 `重新回测`。
12. `开始回测/重新回测` 使用 `buildBacktestExecutionRequestDraft()` 或替代 builder 构造 create request，并附带：
   - `client_request_id`。
   - `rule_hash`。
   - `execution_config_hash`。
   - `period_key`。
   ...
   - optional `range_hint`。
   - `ui_display_snapshot`。
13. `buildBacktestDateRange()` 不再作为 Step 5 create 的权威 range 来源；可以删除，或仅保留在单测里构造非权威 `range_hint`。
14. Step 5 不自动重新执行；用户修改 period/benchmark 后必须点击开始或重新回测。
15. `重新回测` 必须生成新的 `client_request_id`，保留旧 run 作为历史快照，当前视图切到新 run polling。
16. 修改 Step 1/2/4 后：
   - 标记当前 draft stale。
   - 禁用开始/重新回测。
   - 保留已成功 run 的结果但标记“历史快照”，不得把旧结果当作当前草稿结果。
17. result queries 只在 run `succeeded` 且 `current_result_attempt_id` 非空后启用；否则显示状态或空态。
...
8. 首次进入 Step 5 默认选中 `近一年` 并展示后端动态解析的区间。
9. 周期支持 `近一年/近两年/近三年`，三者的区间都来自 options API，不来自浏览器本地日期计算。
10. 修改周期或 benchmark 后，旧结果被标记为历史快照；点击 `重新回测` 后创建并轮询新的 run。
...
2. 在 `/strategies` 按“前端验收样例：低位反转回测”配置代表性规则：
   - Step 1 使用验收样例中的 10 条 AND 过滤条件。
   - Step 2 使用验收样例中的 7 条 conditional points，KDJ 深度/轻度超卖必须互斥。
   - Step 3 成功 preview 并进入非 stale snapshot。
   - Step 4 仓位和费率采用默认值，风控条件全部选择/启用且参数使用默认值。
   - 首次进入 Step 5 时确认默认周期为 `近一年`，默认 benchmark 为 `000300.SH`，页面展示后端动态解析的回测区间。
   - Step 5 使用默认 `近一年 + 000300.SH` 创建 backtest run。
   - 成功后切换到 `近两年` 或 `近三年`，并切换另一个 benchmark，确认旧结果标记为历史快照。
   - 点击 `重新回测` 创建新 run，确认新 run 的 `period_key`、benchmark 和 frozen range 与 pending config 一致。
...
6. 浏览器验收：
   ...
   - 首次进入默认 `近一年`，周期下拉只包含 `近一年/近两年/近三年`，三者区间来自 Rearview options API。
   - 修改周期或 benchmark 后不会自动执行；点击 `重新回测` 才创建新 run。
   - Step 1/Step 2 字段展示使用 canonical 字段：`pct_amplitude`、`close_price_forward_adj`、`boll_lower_20_2` 等，不显示未落地别名 `amplitude`、`forward_close_price`、`boll_dn_20_2`。
...
9. 截图证据链：
   - 按“前端验收样例：低位反转回测”列出的 8 张截图命名保存到 `docs/references/screenshots/racingline/<date>/`。
   - 截图必须覆盖 Step 1 条件、Step 2 得分、Step 3 preview、Step 4 默认配置和风控、Step 5 running、Step 5 succeeded、调仓/绩效、修改周期或 benchmark 后重新回测。
10. 写入 `docs/jobs/reports/` 验收报告，报告必须链接上述截图证据链。

完成标准：

1. Step 1 到 Step 5 在真实 Rearview/NATS/ClickHouse 环境中完成一次默认 `近一年` 动态区间回测，并完成一次 period/benchmark 变更后的重新回测。
2. 结果可通过 Step 5 UI 和 Rearview wrapper API 查询。
3. 无 mock 成功路径。
4. 验收报告包含命令、canonical Step 1/Step 2 字段清单、period key、动态解析日期范围、benchmark、run id、result attempt id、compiled sql hash、data coverage summary 和截图证据链链接。
...
18. 禁止前端用浏览器当前日期或自然年回退结果作为 Step 5 权威回测区间。
19. 禁止修改 period/benchmark 后自动静默重跑；必须由用户点击 `开始回测/重新回测` 创建新 run。
20. 禁止 `重新回测` 复用旧 `client_request_id` 或覆盖旧 run/result attempt。
...
6. `GET /rearview/strategy-backtests/options` 能返回 `近一年/近两年/近三年` 的动态区间，并随 benchmark 覆盖变化重新解析。
7. 首次从 Step 4 进入 Step 5 默认 `近一年`，创建 run 时由 Rearview 固化 `period_key/start_date/end_date/benchmark_security_code`。
8. 修改 period/benchmark 后不自动回测，点击 `重新回测` 创建新 run，旧 run 保留为历史快照。
9. 前端验收样例能按 canonical 字段完成 Step 1/2 输入：`pct_amplitude`、`close_price_forward_adj`、`boll_lower_20_2` 等别名映射正确，KDJ 分段得分互斥。
10. `/rearview/strategy-backtests/{id}/...` wrapper APIs 能读取当前 `result_attempt_id` 的结果。
11. `/rearview/strategy-backtests/{id}/nav` 返回策略和 benchmark 两条净值曲线。
12. `/rearview/strategy-backtests/{id}/rebalance-records` 返回原型持仓记录所需的调仓日和行级字段。
13. Racingline Step 5 移除 mock 成功路径，只展示真实运行状态和结果。
14. Step 5 摘要展示 Step 4 的建仓规则、仓位、买入规则、卖出规则、费率和调仓配置。
15. 回测输出严格裁剪在 `[start_date, end_date]` 内。
16. 卖出后再次入选的证券允许重新买入。
17. MA、MA 组合、EMA 三类指标止损在 worker 行情输入和短样本中通过。
18. worker at-least-once 重投递不会导致 stuck run 或暴露部分写入 attempt。
19. run 保存 compiled SQL/data coverage provenance，能解释信号和结果来源。
20. live smoke 完成并写入 job report，报告链接前端截图证据链。
