# Plan 0061: BaoStock 日 K 统一区间请求与 TCP 超时防护实施计划
...
RFC 0035 确定 BaoStock 日 K asset 不应继续依赖 `mode="daily" | "range_backfill"` 区分增量和回填。Dagster 已经通过 partition selection 提供执行窗口：
...
- RFC：[0035-baostock-daily-kline-unified-range-request.md](../../RFC/archive/0035-baostock-daily-kline-unified-range-request.md)
- 当前实现：
...
- 可选更新 `docs/RFC/0035-baostock-daily-kline-unified-range-request.md`
...
- RFC 0035 的执行入口设计落地：无 `mode`，只由 partition selection 决定请求窗口。
