...
- 不新增：
  ...
  - 父级补充字段
  - 展开索引
- object/struct 按原始字段路径展平成列。
- array/list 保留数组边界；数组元素中的 object/struct 按原始字段路径展平。
- 展平列名必须可逆映射到原始字段路径。
...
info[].time_preview[]
limit_up_count.today.num
result[].imgs
```

如果 PyArrow 或下游工具对点号/方括号列名兼容性不足，实现可以使用等价转义：

```text
list__article__action_info__time
info__time_preview
limit_up_count__today__num
result__imgs
```

但必须在 `schemas.py` 中集中维护转换规则，并在 metadata 中记录 `flatten_column_naming="escaped_path"`。
...
- 一行对应响应 `data[]` 中一个板块内容对象。
- 空 `data=[]` 时写 0 行，保留完整 schema。
- `list[]` 及其内部 `article`、`action_info`、`user` 完整展平。
...
- 一行对应一个分页内容对象 `data`。
- 每页写一行，保留：
  - 分页字段：`pageNo`、`pageSize`、`totalCount`、`totalPages`、`hasNext`、`nextPage` 等。
  - `result[]` 文章内容字段。
...
- `jiuyan__action_field` 展平 `list[].article.action_info.*` 和 `list[].article.user.*`。
...
- `ths__limit_up_pool` 展平 `page.*`、`info[].*`、`limit_up_count.*`、`limit_down_count.*`、`trade_status.*`。
