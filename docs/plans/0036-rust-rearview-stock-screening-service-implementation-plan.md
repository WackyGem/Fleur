...
5. HTTP API 支持创建规则集、创建不可变规则版本、发起运行、查询 run/day/pool/signals，以及 explain。
...
| `POST` | `/rearview/explain` | 校验和解释规则，不写股票池和信号 |
...
5. 第一版可以不实现取消接口，但状态机保留 `cancelled`。
