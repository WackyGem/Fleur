...
本文档定义 fleur 在 `pipeline/contracts` 收敛到 ClickHouse raw 层之后，dbt `stg/int/mart` 层字段命名、字段值格式标准化、字段文档和 raw source 列级治理的新方案。
...
- 这个 canonical field 在 fleur 内表达什么语义？
...
| dbt staging | `security_code` | fleur canonical 字段 |
...
- 日期、时间戳和布尔字段的类型收敛。
- 明确枚举值的轻量标准化，例如供应商交易状态编码 -> fleur 交易状态枚举。
...
models:
  - name: stg_eastmoney__equity_history
    description: EastMoney equity history normalized to fleur canonical fields.
    columns:
      - name: security_code
...
models:
  - name: stg_baostock__daily_prices
    description: BaoStock daily price rows normalized to fleur canonical fields.
    columns:
      - name: security_code
        description: "{{ doc('field_security_code') }}"
        data_type: String
  ...
  meta:
    ...
    derived_from:
      - source: raw
        table: baostock__query_history_k_data_plus_daily
        column: tradestatus
    derivation_note_zh: 将供应商交易状态编码标准化为 fleur 交易状态枚举。
```
