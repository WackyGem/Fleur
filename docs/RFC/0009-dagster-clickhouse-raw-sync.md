...
4. **最新快照资产使用完整快照替换协议**：交易日历、证券基础信息、行业列表等无分区资产按整表快照加载，不暴露半成品。
...
| 类型 | 当前内容 |
|------|----------|
| Assets | 18 个，全部在 group `s3_sources` |
| Jobs | 9 个 source jobs |
| Schedules | 7 个 source schedules |
| Resources | 7 个，无 ClickHouse resource |
...
以下资产第一阶段不纳入 ClickHouse raw 同步：

| 资产 | 原因 |
|------|------|
| `source/jiuyan__industry_images` | 图片对象与 PostgreSQL 状态资产，不是 Parquet 表 |
| `source/jiuyan__industry_ocr` | OCR 状态型结果，需先明确表结构、去重键和更新语义 |
...
`columns` 的最终类型应基于 RFC 0007 的 S3 Parquet schema 类型优化结果。若某个资产仍是全 String Parquet，第一阶段可以先以 String raw 表接入，但必须在 spec 中标记 schema 技术债，避免 dbt staging 误认为类型已经优化完成。
...
| 2 | `sina__trade_calendar`、`baostock__query_stock_basic` 最新快照 | 明确 snapshot 替换 SQL，并完成小表同步 |
| 3 | `jiuyan__action_field_compacted`、`ths__limit_up_pool_compacted` | compacted 年度资产完成后自动同步 ClickHouse |
| 4 | `eastmoney__*` 年度资产 | 批量 spec 生成，完成 schema 类型债务标记或迁移 |
| 5 | dbt raw source/staging/marts | dbt 从 ClickHouse raw 读取，不再直接依赖 S3 |
...
- 原有 18 个 source assets、jobs、schedules 不被破坏；
...
| 风险 | 缓解 |
|------|------|
| Parquet schema 仍是全 String，raw 表类型优化受阻 | spec 标记 schema debt；先 String 接入，再按 RFC 0007 分批迁移 |
...
1. ClickHouse 客户端选型：HTTP client、native client，或现有项目是否引入官方 Python client。
...
5. `eastmoney__*` schema 类型：是否必须先完成 RFC 0007 的 typed Parquet 改造再接入 ClickHouse。
