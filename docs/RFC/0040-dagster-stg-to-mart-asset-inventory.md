...
2026-07-01 补充实现结论：编排层已收敛为一个 daily network controller：`daily__fetch_history_sources_to_marts_schedule_job`。daily network 以每天变动的数据为 root，沿真实 asset dependency 扩展到 staging、intermediate、calculation、wrappers 和 marts。portfolio backtest analytics 和 portfolio live 控制面不纳入该 source-to-marts network。
...
| Rearview | `REARVIEW_DEFS` | `rearview/strategy_portfolio_daily_runs` 控制面资产 |
...
| `rearview` | 1 | strategy portfolio daily run 控制面资产 |
...
`rearview/strategy_portfolio_daily_runs` 是 scheduler 内唯一 Rearview 可执行资产，group 为 `rearview`，每日 `20:00` 由 `portfolio__daily_run_schedule` 触发。它调用 Rearview strategy portfolio daily run APIs，等待 worker 终态，并把 `live_nav_daily`、`live_trade`、`live_closed_trade` 的行数写入 materialization metadata。
...
| 类型 | 名称 | 当前选择范围 |
| --- | --- | --- |
| job | `daily__fetch_history_sources_to_marts_schedule_job` | `target_date` 单日 source -> raw -> stg -> int -> calculation -> mart controller；复用 `backfill__fetch_history_sources_to_marts_job` registry |
...
| job | `strategy_portfolio__daily_run_job` | `rearview/strategy_portfolio_daily_runs` |
| schedule | `portfolio__daily_run_schedule` | 每日 `20:00` 触发 Rearview portfolio daily run |
...
daily source-to-marts network 只覆盖从市场数据 source/raw 出发的 stg -> mart 链路，不纳入 portfolio backtest analytics 或 portfolio live。覆盖方式分两类：
...
| portfolio live | 不纳入第一版 | `rearview/strategy_portfolio_daily_runs` 和 `fleur_portfolio.live_*` 保持独立。live facts 当前不在 Dagster asset 清单中，也不被现有 dbt mart 消费。 |
...
5. backfill、portfolio backtest analytics 和 portfolio live 不在本次清理范围内；它们先保持独立，避免把历史回填和 portfolio 链路混入 daily mart 编排。
...
| `strategy_portfolio__daily_run_job`、`portfolio__daily_run_schedule` | 第一版保留独立 | portfolio live 控制面暂不纳入 daily network。 |
...
2. portfolio live 继续保持独立，除非后续 dbt marts 明确消费 `fleur_portfolio.live_*`。
