...
领域：racingline
关联系统：racingline
代码根：app/racingline_new/, app/racingline/
...
这些问题往往必须通过可运行页面、截图和实际点击才能收敛。Racingline 当前已经存在 `app/racingline/` 旧工程，同时新建了 `app/racingline_new/` 作为并行重构工程，适合把原型和正式替换过程分开。
...
4. 保持 `app/racingline_new/` 与现有 `app/racingline/` 并行，直到新体验被确认可替换。
5. 让正式 plan 从已验证的页面骨架、交互结论和验收标准出发，而不是从纯文字假设出发。
6. 保持 `racingline_new` 与 `racingline` 的技术选型一致；原型工程不是重新选择前端技术栈的入口。
...
`app/racingline_new/` 是 Racingline 的并行原型和重构工程，不是第二个产品线，也不是重新选型工程。除非另起 ADR 或更新 [ADR 0011](../ADR/0011-racingline-frontend-technology-stack.md)，`racingline_new` 的正式实现必须继承现有 `racingline` 技术栈：
...
RFC 阶段可以临时比较字体、主题、密度或组件外观，但这些实验只属于视觉原型材料。任何改变 shadcn style、primitive base、icon library、状态管理、路由、图表库或 package 管理方式的决定，都必须进入 ADR，而不能只靠 `racingline_new` 的脚手架结果自然沉淀。
...
1. 写出用户故事、首屏任务和导航假设。
2. 在 `app/racingline_new/` 中实现最小可运行交互骨架。
...
原型不得直接修改 `app/racingline/` 的生产入口。只有当 RFC 和实施计划确认替换路径后，才允许把 `racingline_new` 作为新生产前端的来源。

如果需要进一步隔离，可以在 `app/racingline_new/src/prototypes/` 或临时 route 中构建原型页面；进入正式实现时再移动到 `src/routes/`、`src/components/racingline/` 和 `src/features/`。
...
`app/racingline_new/` 在用户确认前保持并行工程。替换旧 `app/racingline/` 前，必须完成：
...
1. `app/racingline_new/` 最终是重命名为 `app/racingline/`，还是保留新目录并更新命令？
2. 原型阶段是否需要专门的 screenshot report 模板？
3. 当前 `app/racingline_new/` 脚手架如果与 ADR 0011 存在差异，应恢复到 `racingline` 技术栈，还是另起 ADR 评估变更？
