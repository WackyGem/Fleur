...
17. 提供 `POST /rearview/examples/strategy-portfolios/racingline-0051-low-reversal/ensure`，把固化 example data config 转换为正常 Rearview request DTO，复用 validate/canonicalization 和 `create_strategy_portfolio_from_snapshot` 持久化边界，按 `case_id + version + fixture_hash` 幂等创建或复用 `source_kind = example` 的 strategy portfolio；当前 legacy 0051 path 承载 `racingline_0051_low_reversal` / `v2` 策略搜索低位反转 fixture；该 API 不直接触发 worker，后续清算仍走正式 daily-run range API。
...
`make stop` 只清理前后端 dev server 端口；停止 Docker 依赖服务仍使用 `make down`。
...
| [../jobs/reports/2026-07-03-example-portfolio-live-job-strategy-search-replacement.md](../jobs/reports/2026-07-03-example-portfolio-live-job-strategy-search-replacement.md) | 将策略搜索最佳样本内规则置换为 `example__portfolio_live_job` 使用的 `v2` example fixture |
