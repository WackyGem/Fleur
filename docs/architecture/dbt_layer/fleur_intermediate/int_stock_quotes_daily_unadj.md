...
- Intermediate 设计：`docs/architecture/dbt_layer/fleur_intermediate/int_stock_basic_snapshot.md`
- Staging 设计：`docs/architecture/dbt_layer/fleur_staging/stg_baostock__query_history_k_data_plus_daily.md`
- Intermediate 设计：`docs/architecture/dbt_layer/fleur_intermediate/int_trade_calendar.md`
- Intermediate 设计：`docs/architecture/dbt_layer/fleur_intermediate/int_stock_shares_history.md`
- Intermediate 设计：`docs/architecture/dbt_layer/fleur_intermediate/int_stock_exrights_event.md`
...
| `turnover_rate_pct` | `volume / float_shares_a * 100` | `Nullable(Float64)` | 换手率，百分数口径，`1.23` 表示 `1.23%`。 |
| `turnover_rate_free_float_pct` | `volume / free_float_shares * 100` | `Nullable(Float64)` | 实际换手率，使用 A 股自由流通股本作分母，百分数口径。 |
| `amplitude_pct` | `(high_price - low_price) / prev_close_price * 100` | `Nullable(Float64)` | 振幅，使用 BaoStock `preclose` 口径前收盘价计算，百分数口径。 |
| `change_pct` | `(close_price - prev_close_price) / prev_close_price * 100` | `Nullable(Float64)` | 涨跌幅，使用 BaoStock `preclose` 口径前收盘价计算，百分数口径。 |
...
| `market_cap` | `close_price * shares` | `Nullable(Float64)` | A 股市值，单位元。 |
| `float_market_cap` | `close_price * float_shares_a` | `Nullable(Float64)` | A 股流通市值，单位元。 |
| `free_float_market_cap` | `close_price * free_float_shares` | `Nullable(Float64)` | A 股自由流通市值，单位元。 |
| `shares` | `int_stock_shares_history.shares` | `Nullable(Float64)` | A 股股本，单位股。 |
| `float_shares_a` | `int_stock_shares_history.float_shares_a` | `Nullable(Float64)` | A 股流通股，单位股。 |
| `free_float_shares` | `int_stock_shares_history.free_float_shares` | `Nullable(Float64)` | A 股自由流通股，单位股。 |
| `dy_static_pct` | 最近年度现金分红 / `close_price * 100` | `Nullable(Float64)` | 股息率（静），百分数口径。 |
| `dy_ttm_pct` | 近 12 个月现金分红 / `close_price * 100` | `Nullable(Float64)` | 股息率（TTM），百分数口径。 |
...
4. 成交与日频交易指标：`volume`, `amount`, `turnover_rate_pct`, `turnover_rate_free_float_pct`, `amplitude_pct`, `change_pct`
...
7. 股息率：`dy_static_pct`, `dy_ttm_pct`
...
`shares`、`float_shares_a`、`free_float_shares` 直接透传该区间表字段。若同一证券同一交易日无法命中股本区间，股本、市值和换手率字段均输出 `NULL`，行情行仍保留。

### 5.2 换手率、涨跌幅和振幅

```text
turnover_rate_pct = volume / float_shares_a * 100
turnover_rate_free_float_pct = volume / free_float_shares * 100
amplitude_pct = (high_price - low_price) / prev_close_price * 100
change_pct = (close_price - prev_close_price) / prev_close_price * 100
```
...
- `amplitude_pct` 和 `change_pct` 使用 BaoStock source-local `prev_close_price`，与交易所涨跌幅和涨跌停参考价口径保持一致。
...
float_market_cap = close_price * float_shares_a
free_float_market_cap = close_price * free_float_shares
```
...
- `dy_static_pct` 使用同一证券 `ex_dividend_date <= trade_date` 的最近年度现金分红。年度现金分红定义为 `report_date` 为自然年末，即 `toMonth(report_date) = 12 and toDayOfMonth(report_date) = 31` 的现金分红事件；同一 `security_code + report_date` 如有多条现金分红事件，先按报告期求和。
- `dy_ttm_pct` 使用 `trade_date - interval 1 year < ex_dividend_date <= trade_date` 的现金分红事件合计。
- 股息率使用 `close_price` 作分母，保留百分数口径。
- 当现金分红输入缺失时输出 `NULL`；当现金分红为 `0` 时可输出 `0`。
- 当 `close_price` 为 `NULL`、`0` 或小于 `0` 时，`dy_static_pct` 和 `dy_ttm_pct` 输出 `NULL`。
...
quotes_with_metrics as (
    select
        ...
        if(
            ...
            or quotes_with_shares.float_shares_a is null
            or quotes_with_shares.float_shares_a <= 0,
            cast(null, 'Nullable(Float64)'),
            quotes_with_shares.volume / quotes_with_shares.float_shares_a * 100
        ) as turnover_rate_pct,
        if(
            quotes_with_shares.volume is null
            or quotes_with_shares.free_float_shares is null
            or quotes_with_shares.free_float_shares <= 0,
            cast(null, 'Nullable(Float64)'),
            quotes_with_shares.volume / quotes_with_shares.free_float_shares * 100
        ...
        if(
            ...
            or quotes_with_shares.shares is null
            or quotes_with_shares.shares <= 0,
            cast(null, 'Nullable(Float64)'),
            quotes_with_shares.close_price * quotes_with_shares.shares
        ) as market_cap,
        if(
            quotes_with_shares.close_price is null
            or quotes_with_shares.close_price < 0
            ...
            or quotes_with_shares.float_shares_a <= 0,
            cast(null, 'Nullable(Float64)'),
            quotes_with_shares.close_price * quotes_with_shares.float_shares_a
        ) as float_market_cap,
        if(
            quotes_with_shares.close_price is null
            or quotes_with_shares.close_price < 0
            ...
            or quotes_with_shares.free_float_shares <= 0,
            cast(null, 'Nullable(Float64)'),
            quotes_with_shares.close_price * quotes_with_shares.free_float_shares
        ) as free_float_market_cap,
        quotes_with_shares.shares,
        quotes_with_shares.float_shares_a,
        quotes_with_shares.free_float_shares,
        if(
...
- 不使用 BaoStock staging 的 source-local `turn` 或 `pctChg`；本模型统一按项目口径重算换手率，并基于 BaoStock `prev_close_price` 重算 `amplitude_pct` 和 `change_pct`。
- 不直接从分红 staging 计算 `dy_static_pct` 和 `dy_ttm_pct`；股息率只依赖已结构化的 `int_stock_exrights_event`。
...
- 增加定向数据测试：
  ...
  - `dy_static_pct` 使用的年度现金分红除权除息日不得大于当前 `trade_date`。
  - `dy_ttm_pct` 只聚合当前 `trade_date` 前 12 个月内已除权除息的现金分红事件。
