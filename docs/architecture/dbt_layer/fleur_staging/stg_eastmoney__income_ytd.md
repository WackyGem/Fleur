# stg_eastmoney__income_ytd 设计

状态：Design

依据：

- Raw profile：`docs/references/raw_profile/eastmoney__income_ytd.md`
- 数据字典：`docs/references/data_dict/eastmoney__income_ytd.md`
- Raw source：`source('raw', 'eastmoney__income_ytd')`
- 目标位置：`pipeline/elt/models/staging/eastmoney/stg_eastmoney__income_ytd.sql`

## 1. 模型定位

EastMoney F10 利润表年初至报告期末累计口径的 source-local staging model。模型保留东方财富披露的一证券一报告期宽表记录，把证券代码、报告期、公告日期、累计利润表科目金额、同比增长率和审计意见字段整理为 dbt 可复用的字段名与类型。

staging 只做确定性的字段命名、证券代码标准化、基础类型保留和 source-local NULL 语义保留，不做财务科目重算、累计与单季度口径互推、报表勾稽校验、跨源财报合并、公告版本选择或宽表拆长表。

## 2. 数据特征

- 行数：298,396。
- 覆盖证券：`SECUCODE` 5,524 个；`SECURITY_CODE` 5,524 个。
- 粒度：一行代表一个 `SECUCODE`, `REPORT_DATE` 年初至报告期末累计利润表记录。
- 候选键：`SECUCODE`, `REPORT_DATE`，profile 未发现重复。
- `SECUCODE` 全部为 `600000.SH` 类 canonical 后缀格式；`SECURITY_CODE` 全部为 6 位本地代码。
- 日期范围：`REPORT_DATE` 为 1988-12-31 至 2026-03-31；`NOTICE_DATE` 为 1991-06-10 至 2026-05-15；`UPDATE_DATE` 为 1991-06-10 至 2026-06-02。
- `REPORT_DATE`, `NOTICE_DATE` 无 NULL；`UPDATE_DATE` 有 523 行 NULL；日期字段未发现 `1970-01-01` 占位值。
- `REPORT_TYPE` 仅观察到 `年报`、`中报`、`一季报`、`三季报`。
- `ORG_TYPE` 观察到 `通用` 292,603 行、`证券` 2,826 行、`银行` 2,449 行、`保险` 518 行。
- `CURRENCY` 仅观察到 `CNY` 和 NULL，其中 NULL 878 行；staging 不用 `CNY` 回填。
- raw 共 203 个字段，其中 13 个元数据字段、190 个数值字段。数值字段分为 95 个累计绝对值字段和 95 个 `_YOY` 同比增长率字段，无 `_QOQ` 环比字段。
- 190 个数值字段中 174 个出现负值、162 个出现 0 值、112 个 NULL 数不低于 80%。这些值符合利润表科目、调整项、行业适用性和同比增长率口径的 source-local 特征，staging 不过滤、不置零、不转 NULL。

## 3. 字段设计

第一版建议输出 raw 中 203 个字段的信息：`SECUCODE` 标准化为 `security_code`，`SECURITY_CODE` 暴露为 `security_local_code`，其余字段按规则改为 lower snake case。不要额外输出 `exchange_code`；下游如需要可从 `security_code` 派生。
...
| 证券属性 | `org_type` | `ORG_TYPE` | `LowCardinality(String)` | 保留供应商机构类型枚举。 |
| 报告期 | `report_date` | `REPORT_DATE` | `Date` | 财报报告期截止日，进入候选键。 |
| 报告期 | `report_type` | `REPORT_TYPE` | `LowCardinality(String)` | 累计口径报告类型，保留 `一季报`、`中报`、`三季报`、`年报`。 |
| 报告期 | `report_date_name` | `REPORT_DATE_NAME` | `LowCardinality(String)` | 报告期展示标签，例如 `2026一季报`。 |
...
| 披露时间 | `update_date` | `UPDATE_DATE` | `Nullable(Date)` | 更新日期，523 行 NULL 保留。 |
...
| 累计金额/每股值 | `<line_item>` | 非 `_YOY` 数值字段 | `Nullable(Float64)` | 95 个年初至报告期末累计利润表绝对值字段，金额单位按 raw 保留；EPS 字段按 raw 元/股保留。 |
| 同比增长率 | `<line_item>_yoy` | `_YOY` 数值字段 | `Nullable(Float64)` | 95 个同比增长率字段，保留供应商百分数口径，不除以 100。 |

数值字段命名规则：

- 常规字段采用确定性 lower snake case，例如 `TOTAL_OPERATE_INCOME` -> `total_operate_income`，`TOTAL_OPERATE_INCOME_YOY` -> `total_operate_income_yoy`。
- 保留 source-local 科目粒度，不把宽表拆成长表，不把相近科目合并。
- 金额、EPS 和同比字段必须在 YAML `config.meta.source_columns` 中记录原始字段；`_YOY` metadata 需标记 `unit: percent` 或等价说明。
...
- 0 值是披露事实，不转 NULL；负值可能来自费用冲回、减值转回、亏损、少数股东损益、综合收益调整或同比下降，不在 staging 过滤。
...
- 不用累计表反推单季度利润表，也不用单季度表反推累计利润表；`income_ytd` 和 `income_sq` 的口径对齐延后到 intermediate。
...
3. 利润表累计绝对值字段：所有非 `_YOY` 数值字段，按 data_dict 原始顺序输出。
4. 同比增长率字段：所有 `_YOY` 数值字段，按 data_dict 原始顺序输出。
5. 审计意见字段：`opinion_type`。
...
- `org_type`: 可加 `accepted_values` 取 `通用`, `证券`, `银行`, `保险`。
- `security_type_code`: 不建议第一版加 accepted-values；profile 观察到 `058001001` 和 `058001008`，枚举语义需等证券类型维度设计确认。
- `opinion_type`: 不加 `not_null`；accepted-values 需先补充完整枚举画像。
- 利润表科目、EPS 和 `_YOY` 字段不加全字段 `not_null`、非负或阈值测试；高 NULL、0、负值和极端增长率均已由 profile 证明需要保留。

## 7. YAML metadata 建议

- 每个输出字段都记录 `config.meta.source_columns`，source 为 `raw`，table 为 `eastmoney__income_ytd`。
...
- `security_local_code` 记录 `glossary_key: security_local_code` 和 normalization metadata：`macro: cn_security_local_code`，`input_format: a_share_local_code`。
- 其余 source-local 字段记录 `dictionary_scope: local`。
- 累计金额字段记录 source-local 单位说明，例如 `unit: raw_currency_amount`；不要在 metadata 中声称已做元/万元/亿元转换，除非后续 profile 明确验证。远端接口文档说明绝对值字段为金额元，但当前 staging 仍以 raw currency amount 表达，避免与多币种或供应商变更耦合。
...
- 年初至报告期末累计利润表与单季度利润表的口径对齐、互推和差异解释。
