...
用户维护的环境变量统一配置在根目录 `.env` 文件中，URL 形态的本地运行变量可由 Makefile 或工具入口派生。业务模块不要直接读取环境变量，应通过 `config/`、resource、factory 或 gateway 注入配置。

- `RUSTFS_*`：S3 兼容对象存储（RustFS/MinIO）
- `BAOSTOCK_*`：BaoStock TCP 连接配置
- `PIPELINE_DATABASE_URL`：运行时派生的 PostgreSQL 连接字符串（OCR 状态管理）
- `JIUYAN_*`：聚源数据 API 认证
