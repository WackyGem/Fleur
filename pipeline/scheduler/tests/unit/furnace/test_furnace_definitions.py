...
def test_furnace_jobs_select_only_kdj_asset() -> None:
    ...
    assert all(str(job.selection) == 'key:"fleur_calculation/calc_stock_kdj_daily"' for job in jobs)
...
def test_furnace_daily_schedule_uses_append_latest_config() -> None:
    ...
    assert tick.run_requests[0].run_config["ops"]["furnace__calc_stock_kdj_daily"]["config"][
        "mode"
    ] == "append-latest"
