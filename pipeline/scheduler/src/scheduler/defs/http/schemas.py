...
from scheduler.defs.contract_schemas import (
    JIUYAN_ACTION_FIELD_SCHEMA,
    JIUYAN_INDUSTRY_LIST_SCHEMA,
    THS_LIMIT_UP_POOL_SCHEMA,
)

FLATTEN_COLUMN_NAMING = "shortest_leaf"

JIUYAN_ACTION_FIELD_COLUMNS = tuple(JIUYAN_ACTION_FIELD_SCHEMA.names)
...
THS_LIMIT_UP_POOL_COLUMNS = tuple(THS_LIMIT_UP_POOL_SCHEMA.names)
THS_LIMIT_UP_POOL_INFO_COLUMNS = tuple(
    column for column in THS_LIMIT_UP_POOL_COLUMNS if column != "date"
)

JIUYAN_INDUSTRY_LIST_COLUMNS = tuple(JIUYAN_INDUSTRY_LIST_SCHEMA.names)
...
def empty_jiuyan_action_field_table() -> pa.Table:
    return rows_to_typed_table([], JIUYAN_ACTION_FIELD_SCHEMA)
