...
from scheduler.defs.http_resources.jiuyan__action_field import (
    MarketEventBackfillConfig,
    _http_stats_metadata,
)
...
async def _materialize_limit_up_pool_range(
    ...
    async with AioHttpClient(
        ...
        result = await materialize_trade_date_range(
            ...
            fetch_table_for_trade_date=lambda trade_date: _fetch_limit_up_pool_table(
                client,
                trade_date=trade_date,
            ),
        )
        result.metadata.update(_http_stats_metadata(client))
        return result


async def _fetch_limit_up_pool_table(
    client: AioHttpClient,
    *,
    trade_date: date,
) -> tuple[pa.Table, Mapping[str, object]]:
    started_at = time.perf_counter()
    pages: list[Mapping[str, object]] = []
    seen_info_fingerprints: set[str] = set()
    duplicate_page_row_count = 0
    page_number = 1
    total_pages = 1
    ...
    while page_number <= total_pages:
        payload = await client.request_json_object(
            HttpRequest(
                ...
                params=_limit_up_pool_params(trade_date=trade_date, page_number=page_number),
            )
        )
        ...
        if not isinstance(page, Mapping):
            ...
            raise RuntimeError(msg)
        if page_number == 1:
            total_pages = _positive_int_or_default(page.get("count"), default=1)
        info = data.get("info")
    ...
    return table_result.table, {
        ...
        "empty_response_count": 1 if not seen_info_fingerprints else 0,
        "page_count": len(pages),
        "page_total": total_pages,
        "duplicate_page_row_count": duplicate_page_row_count,
        "source_response_date": source_response_date,
        "trade_status": last_page.get("trade_status"),
...
def _limit_up_pool_params(*, trade_date: date, page_number: int) -> dict[str, str]:
    return {
