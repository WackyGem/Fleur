...
from scheduler.defs.config import JIUYAN_COOKIE, JIUYAN_TOKEN
...
async def _materialize_action_field_range(
    ...
    async with AioHttpClient(
        ...
        result = await materialize_trade_date_range(
            ...
            fetch_table_for_trade_date=lambda trade_date: _fetch_action_field_table(
                client,
...
async def _fetch_action_field_table(
...
def _http_stats_metadata(client: AioHttpClient) -> dict[str, int]:
    return {
        "request_count": client.stats.request_count,
        "retry_count": client.stats.retry_count,
        "transient_error_count": client.stats.transient_error_count,
        "http_4xx_count": client.stats.http_4xx_count,
        "http_5xx_count": client.stats.http_5xx_count,
        "decode_error_count": client.stats.decode_error_count,
    }
