...
def empty_jiuyan_action_field_table() -> pa.Table:
    return _empty_table(JIUYAN_ACTION_FIELD_COLUMNS)
...
def jiuyan_action_field_to_table(
    ...
    for content_row in content_rows:
        unknown_field_count += _unknown_count(
            content_row,
        ...
        for stock in list_value:
            ...
            _copy_selected(output_row, content_row, JIUYAN_ACTION_FIELD_OUTER_COLUMNS)
            _copy_selected(output_row, stock, JIUYAN_ACTION_FIELD_STOCK_COLUMNS)
            _copy_selected(output_row, action_info, JIUYAN_ACTION_FIELD_ACTION_INFO_COLUMNS)
            rows.append(output_row)

    return TableConversionResult(
        table=_rows_to_string_table(rows, JIUYAN_ACTION_FIELD_COLUMNS),
...
def ths_limit_up_pool_to_table(
    ...
    for page in pages:
        ...
        for item in info:
            ...
            _copy_selected(output_row, item, THS_LIMIT_UP_POOL_INFO_COLUMNS)
            rows.append(output_row)

    return TableConversionResult(
        table=_rows_to_string_table(rows, THS_LIMIT_UP_POOL_COLUMNS),
        unknown_field_count=unknown_field_count,
    )
...
def jiuyan_industry_list_to_table(
    ...
    for page in pages:
        ...
        for item in result:
            ...
            _copy_selected(output_row, item, JIUYAN_INDUSTRY_LIST_COLUMNS)
            rows.append(output_row)

    return TableConversionResult(
        table=_rows_to_string_table(rows, JIUYAN_INDUSTRY_LIST_COLUMNS),
        unknown_field_count=unknown_field_count,
    )


def _empty_table(columns: Sequence[str]) -> pa.Table:
    return _rows_to_string_table([], columns)


def _rows_to_string_table(
    rows: Sequence[Mapping[str, object]],
    columns: Sequence[str],
) -> pa.Table:
    arrays = {
        column: pa.array(
            [_string_or_null(row.get(column)) for row in rows],
            type=pa.string(),
        )
        for column in columns
    }
    return pa.table(arrays, schema=_string_schema(columns))
