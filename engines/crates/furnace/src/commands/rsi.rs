use furnace_io::{
    DEFAULT_INPUT_TABLE, DEFAULT_RSI_OUTPUT_TABLE, DEFAULT_RSI_PRICE_COLUMN, RsiRunRequest,
    RsiWriteMode,
};
...
pub(crate) struct RsiCommandConfig {
    common: CommonCommandOptions<RsiWriteMode>,
    input_table: String,
    output_table: String,
    price_column: String,
}
...
impl RsiCommandConfig {
    pub(crate) fn parse(args: impl IntoIterator<Item = String>) -> Result<Self, CliError> {
        let mut common = CommonCommandOptionsBuilder::new(RsiWriteMode::DryRun);
        ...
        while let Some(flag) = args.next() {
            ...
            if common.apply_flag(&flag, value.clone(), |value| {
                parse_mode(value, RsiWriteMode::parse)
            })? {
                continue;
            }
    ...
    pub(crate) fn to_request(&self) -> RsiRunRequest {
        RsiRunRequest {
