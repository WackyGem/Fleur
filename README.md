...
> [!IMPORTANT]
> **本项目为模拟项目**，不涉及资金交易，所有数据、指标、策略与 AI 输出仅供学习参考, 不保证数据真实有效 , **不构成任何投资建议**，据此操作的风险与损失由使用者自行承担。请务必阅读 [`风险提示与免责声明`](SECURITY.md#风险提示与免责声明)。
...
| 层级 | 技术栈 |
|---|---|
| 语言与包管理 | Python 3.12.13（`uv`）、Rust 2024 edition（Cargo）、TypeScript（npm 11.13.0） |
| 数据编排与建模 | Dagster 1.13.6、dagster-dbt 0.29.6、dbt Core 1.11、dbt-clickhouse |
| 数据存储与消息 | RustFS（S3 兼容对象存储）、ClickHouse、PostgreSQL 18、NATS JetStream |
| Python 数据层 | PyArrow、clickhouse-connect、aiohttp、psycopg、Pydantic、PyYAML、Alembic |
| Rust 计算与服务 | Tokio、Axum、SQLx/PostgreSQL、async-nats、ClickHouse Rust client、Rayon、Serde |
| 前端工作台 | Vite、React 19、React Router 7、TanStack Query 5、Zustand、Tailwind CSS 4、shadcn/ui、Base UI、lightweight-charts |
| 部署与网关 | Docker Compose、Nginx 1.29、Node 24 构建镜像、Python 3.12 slim 镜像、Rust 1 bookworm 构建镜像 |
| 质量门禁 | Ruff、Pyright、pytest、Cargo fmt / Clippy / test、ESLint、TypeScript、Vitest |
