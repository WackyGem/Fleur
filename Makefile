...
RUSTFS_API_PORT ?= 34050
RUSTFS_ENDPOINT ?= http://127.0.0.1:$(RUSTFS_API_PORT)
CLICKHOUSE_HTTP_PORT ?= 34052
...
POSTGRES_PORT ?= 34054
# Keep these runtime URLs derived from POSTGRES_* so .env only needs the
# PostgreSQL connection parts users actually edit.
...
REARVIEW_API_BASE_URL ?= http://$(REARVIEW_HTTP_BIND)
VITE_REARVIEW_API_BASE_URL ?= $(REARVIEW_API_BASE_URL)
NATS_CLIENT_PORT ?= 34055
REARVIEW_NATS_URL ?= nats://127.0.0.1:$(NATS_CLIENT_PORT)
export \
	CLICKHOUSE_HOST CLICKHOUSE_PORT CLICKHOUSE_S3_ENDPOINT \
	PIPELINE_DATABASE_URL REARVIEW_API_BASE_URL REARVIEW_DATABASE_URL \
	REARVIEW_NATS_URL RUSTFS_ENDPOINT VITE_REARVIEW_API_BASE_URL
